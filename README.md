...
├── store/
│   ├── atoms.ts           # Jotai atoms (탭, AI, 검색, 대시보드)
...
```
keyword (기본)  → Fuse.js 퍼지 매칭 (title, tags, content)
```
...
| 15 | 키워드 검색 (Fuse.js) + 콘텐츠 요약 (Claude API) | ✅ 완료 |
| 16 | Document 타입 — HybridContent, 5종 섹션, DocumentEditor, DnD 정렬 | ✅ 완료 |
| 17 | Phase C 구조적 리디자인 (XSS 수정, 타입 통합, 에디터 미리보기, 백업 알림) | ✅ 완료 |
| 18 | Phase D Document 섹션 품질 향상 (템플릿, Env 일괄입력, 언어 확장 등) | 🔲 예정 |
...
| 8 | 키워드 검색 | Fuse.js 퍼지 매칭 (title, tags, content) | P1 | ✅ 완료 |
...
| 14 | Document 템플릿 프리셋 | 빈 문서/서버접속정보/API문서/프로젝트설정 등 프리셋 선택으로 빠른 생성 | P1 | 🔲 예정 |
| 15 | Env 섹션 일괄 붙여넣기 | .env 파일 내용 textarea에 붙여넣기 → KEY=VALUE 자동 파싱 | P1 | 🔲 예정 |
| 16 | Markdown 섹션 미리보기 | document 내 markdown 섹션에 소스/미리보기 토글 추가 | P2 | 🔲 예정 |
| 17 | Code 섹션 언어 확장 | 4종(text/bash/sql/json) → yaml/python/js/ts/html/css/dockerfile 등 추가 | P2 | 🔲 예정 |
| 18 | 코드 품질 정리 | ICON_MAP 4중 중복 통합, isSafeUrl 통합, UrlEntry.auth 제거, exhaustive check | P2 | 🔲 예정 |
| 19 | 섹션 복제 기능 | SectionWrapper 메뉴에 "복제" 옵션 추가 (deep copy + 새 id) | P3 | 🔲 예정 |
| 20 | Credential extra → textarea | SSH 키 등 다중 라인 데이터 가독성 개선 | P3 | 🔲 예정 |
| 21 | 공지사항 모달 | 웹 초회접근 시 공지사항 모달 자동 표시 + "오늘 더이상 보지 않기" (localStorage 24h) | P1 | 🔲 예정 |
| 22 | 릴리즈노트 표시 | 공지사항 모달 내 최근 2-3건 릴리즈노트 출력 (TypeScript 상수 배열) | P1 | 🔲 예정 |
| 23 | 사용설명서 가이드 | "사용방법 보기" 링크 → 슬라이드 스테퍼 가이드 모달 (4-6 슬라이드) | P1 | 🔲 예정 |
...
**AI/시맨틱 검색 제거, 키워드 검색 단순화** (PR #8)
- `embeddings.ts`, AI 자연어 검색, 시맨틱 검색, `@huggingface/transformers` 의존성 제거
- AppHeader 3모드(keyword/ai/semantic) → 키워드 검색 단일 모드로 단순화
- Fuse.js 검색 인덱스에 content 필드 포함 (title/tags만 → title/tags/searchText)

**Phase C: 구조적 리디자인** (5 SubTask)
- XSS 수정: `UrlSectionView.tsx`에 `isSafeUrl()` 프로토콜 검증 추가
- EnvSectionView `key={idx}` → `key={entry.id}` (EnvEntry에 nanoid id 필드 추가)
- `navigator.storage.persist()` App.tsx 초기화 시 호출
- 백업 알림: lastExportAt 기반 7일 경과 또는 미백업+10건 이상 시 toast
- Note/Custom 타입 → Markdown 통합 (DB v11 마이그레이션, 12파일 수정)
- Markdown 에디터에 미리보기 토글 추가 (MarkdownEditorWithToggle)
- Document 카드 Preview 개선 (getSectionSummary 섹션별 요약)

**Document View Modal** (`CardFloatingView.tsx`)
- HybridDocumentView: 5종 섹션 읽기 전용 렌더러
- 섹션별 독립 접기/펼치기, 복사 버튼, 비밀번호/secret 마스킹

**섹션 에디터 개선**
- MarkdownSectionView: 하단 드래그 바로 높이 조절 (min 60px)
- CodeSectionView: ResizableMiniCodeEditor 래퍼, 하단 드래그 바 높이 조절 (min 60px)
- UrlSectionView: URL별 메모카드(noteCards) 기능 추가

**DB v11 마이그레이션**
- `note`/`custom` → `markdown` 자동 변환
- `LEGACY_TYPE_MAP`에 note/custom 추가
...
| AppHeader 과부하 | AppHeader.tsx 과부하 (탭+검색+필터) → 컴포넌트 분리 필요 | 리팩토링 예정 |
| Smart Paste api 오탐 | URL 1개만 있어도 api 타입으로 감지 → 인증정보 동반 시에만 감지하도록 수정 필요 | TODO 예정 |
| ICON_MAP 중복 | 4곳에서 동일 Record 중복 정의 (CardFormModal, CardDetailEditor, InfoCard, CardFloatingView) | TODO 예정 |
