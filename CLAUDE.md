...
- 팔레트 유틸 잔존 **10줄 / 19개소 / 6파일** — 전부 알파 변조(`bg-red-500/10` `border-yellow-500/40` 등 배너·뱃지 틴트). 대응하는 알파 틴트 토큰이 없어 **보류**. 해소하려면 시맨틱 틴트 토큰 세트(error/warning/success × bg/border)를 신설해야 하는데, 현재 렌더값과 동일하게 넣으면 테마 무관 상수라 토큰의 의미가 약하다 — **테마별 값을 정할 근거가 생길 때 착수**
  - ⚠️ **"알파 없는 안전 부분집합만 먼저 치운다"는 경로는 없다(2026-08-27 실측: 0건)** — 이 19개소가 잔존 팔레트 유틸의 *전부*이고, 각 줄의 텍스트 색은 이미 `text-[var(--text-*)]`로 토큰화가 끝나 배경/테두리 틴트만 남았다. 부분 청소를 시도하려던 defer는 이 실측으로 **철회**됐다. 다시 조사하지 말 것
