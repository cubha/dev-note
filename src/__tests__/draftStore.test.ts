...
describe('draftStore — drafts 테이블 IO', () => {
  ...
  it('saveDraft → loadDraft 왕복', async () => {
    await saveDraft(1, { title: '제목', type: 'server', tags: 'a, b', body: { kind: 'fields', fields: [['host', '1.1.1.1']], editorText: '메모' } })
  ...
  it('saveDraft를 같은 itemId로 다시 호출하면 upsert', async () => {
    await saveDraft(1, { title: 'v1', type: 'note', tags: '', body: { kind: 'fields', fields: [], editorText: '' } })
    await saveDraft(1, { title: 'v2', type: 'note', tags: '', body: { kind: 'fields', fields: [], editorText: '' } })
  ...
  it('deleteDraft 후 loadDraft는 undefined', async () => {
    await saveDraft(1, { title: 't', type: 'note', tags: '', body: { kind: 'fields', fields: [], editorText: '' } })
    await deleteDraft(1)
    expect(await loadDraft(1)).toBeUndefined()
  })

  it('deleteDrafts로 여러 건 일괄 삭제', async () => {
    await saveDraft(1, { title: 'a', type: 'note', tags: '', body: { kind: 'fields', fields: [], editorText: '' } })
    await saveDraft(2, { title: 'b', type: 'note', tags: '', body: { kind: 'fields', fields: [], editorText: '' } })
    await saveDraft(3, { title: 'c', type: 'note', tags: '', body: { kind: 'fields', fields: [], editorText: '' } })
  ...
  it('listDraftItemIds는 현재 드래프트가 있는 모든 itemId를 반환', async () => {
    await saveDraft(5, { title: 'x', type: 'document', tags: '', body: { kind: 'document', sections: [] } })
    await saveDraft(7, { title: 'y', type: 'db', tags: '', body: { kind: 'fields', fields: [], editorText: '' } })
