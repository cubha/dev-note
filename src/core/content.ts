 ...
 */
export const parseContent = (text: string | null): CardContent => {
  if (!text || text.trim() === '') {
 ...
 */
export const serializeContent = (content: StructuredContent | HybridContent): string => {
 ...
 */
export const createEmptyStructuredContent = (type: ItemType): StructuredContent => {
 ...
 * 기본 마크다운 섹션 1개 포함.
 */
export const createEmptyHybridContent = (): HybridContent => {
  return {
    format: 'hybrid',
    sections: [
      {
 ...
 * (Fuse.js 검색 인덱싱에 사용)
 */
export const extractSearchText = (content: CardContent): string => {
...
const extractHybridSearchText = (sections: AnySection[]): string => {
  const parts: string[] = []
 ...
 * 타입에 맞는 빈 섹션을 생성한다.
 */
export const createSection = (type: SectionType): AnySection => {
 ...
 */
export const getFieldValue = (content: CardContent, key: string): string => {
